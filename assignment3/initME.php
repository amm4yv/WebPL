<?php
    ...
    if (flock($fileptr, LOCK_SH)) {         
      while ($line = fgetss($fileptr)){
        ...
        $insertQuery = "INSERT into Admin VALUES ('$data[0]', '$data[1]', '$data[2]', '$password', NULL)";
    ...
    if (flock($fileptr, LOCK_SH)) {         
      while ($line = fgetss($fileptr)){
        ...
        $insertQuery = "INSERT into Sender VALUES ($data[0], '$data[1]', '$data[2]', '$password', NULL)";
        $db->query($insertQuery);
      }
    }       
    flock($fileptr, LOCK_UN);
    ...
    if (flock($fileptr, LOCK_SH)) {         
      while ($line = fgetss($fileptr)){
        ...
        $ticketQuery = "INSERT into Tickets VALUES ($data[0], '$created_date', $data[1], '$data[2]', $data[3], '$data[4]', '$data[5]')";
